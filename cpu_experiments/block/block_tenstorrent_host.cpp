...
void TT_printBuffer(uint32_t* buffer, size_t size) {
    for(int i = 0; i<size; i++){
        std::pair<bfloat16, bfloat16> two_bfp16 = extract_bfloat16_pair(buffer[i]);
        std::cout << two_bfp16.first.to_float() << " ";
        std::cout << two_bfp16.second.to_float() << " ";
    }
    std::cout << endl;
...
int main(int argc, char** argv) {
    ...
    constexpr CoreCoord all_device_cores = {0,0};//CoreRange({0, 0}, {num_cores_x - 1, num_cores_y - 1});
    ...
    TT_printBuffer(h_scalar_buffer, h_output_mat.size()*h_output_mat[0].size());
    ...
    for(uint32_t i = 0; i < num_cores_total; i++) {
        ...
        CoreCoord core = {0, 0}; // Compute the core coordinates
    ...
    std::cout << h_output_mat.size() << " " << h_output_mat[0].size() << std::endl;
    std::cout << "Output: " << std::endl;
    //fromBufferToAbstraction(h_output_buffer, h_output_mat, h_output_mat.size(), h_output_mat[0].size());
    TT_printBuffer(h_output_buffer, h_output_mat.size()*h_output_mat[0].size());
