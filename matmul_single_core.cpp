...
// QUESTA È LA VERA FUNZIONE CHE SCAMBIA TRA HOST E DEVICE
...
void matmul_single_core(
    ...
    Program program{};              
    CoreRange core({0, 0}, {0, 0});  // SELEZIONI IL NUMERO DI CORES
    ...
    // QUI STO EFFETTIVAMENTE ALLOCANDO I BUFFER NELLA DRAM DEL DEVICE
    ...
    // PERCHÈ VENGONO CREATI ANCHE DEI CIRCULAR BUFFER IN CACHE?
    ...
    // PERCHÈ NON LI SETTO COME NEL COMPUTER KERNEL??

    // PROBABILMENTE L'ORDINE IN CUI CREO I KERNEL È L'ORDINE DI ESECUZIONE

    /* Launch program & read in output buffer result into the host vector */
    EnqueueWriteBuffer(cq, src0_dram_buffer, a.data(), false);  // QUESTA PENSO SIA UNA MEMCPY
    EnqueueWriteBuffer(cq, src1_dram_buffer, b.data(), false);  // QUESTA PENSO SIA UNA MEMCPY

    EnqueueProgram(cq, program, false);  // QUI LANCIO IL KERNEL, BLOCCANTE?
    EnqueueReadBuffer(cq, dst_dram_buffer, output.data(), true); // QUI ATTENDO IL RISULTATO, QUESTA SICURO BLOCCANTE!
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // TO USE A KERNEL WE ALWAYS NEED TO SETUP A DEVICE,

        /* Silicon accelerator setup */
        constexpr int device_id = 0;
        IDevice* device = CreateDevice(device_id); // WE HAVE ONLY ONE DEVICE!


        // INITIALIZATION OF MY PARAMETERS THAT I WILL PASS TO THE FUNCTION -- START
        ...
        // la funzione di tilizing (penso) si occupi di suddividere il buffer in chucks per poi assegnare ogni chuck ad un core
        // nel nostro caso non è troppo importante perchè in questa versione viene utilizzato solamente un singolo core
